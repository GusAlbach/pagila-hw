...
$$
SELECT category.name, COUNT(language.name) FROM category JOIN film_category USING (category_id) jOIN film USING (film_id) JOIN language USING (language_id) WHERE language.name LIKE  $1 || '%' GROUP BY category.name ORDER BY name;
$$
LANGUAGE SQL
IMMUTABLE
